...
function Settimana(props) {
    ...
    function assignIcon(data) {
        data.map((mappedData) => {
            ...
            } else if(mappedData.codice_previsione === 803 || mappedData.codice_previsione === 804) {
                mappedData.weatherIcon = brokenClouds;
            } else if(mappedData.codice_previsione >= 500 && mappedData.codice_previsione <= 504) {
                mappedData.weatherIcon = rain;
            } else if(mappedData.codice_previsione >= 300 && mappedData.codice_previsione <= 321) {
                mappedData.weatherIcon = showerRain;
            } else if(mappedData.codice_previsione >= 200 && mappedData.codice_previsione <= 232) {
                mappedData.weatherIcon = thunderstorm;
            } else if(mappedData.codice_previsione === 511 ) {
                mappedData.weatherIcon = snow;
            } else if(mappedData.codice_previsione >= 520 && mappedData.codice_previsione <= 531) {
                mappedData.weatherIcon = showerRain;
            } else if(mappedData.codice_previsione >= 600 && mappedData.codice_previsione <= 622) {
                mappedData.weatherIcon = snow;
            } else if(mappedData.codice_previsione >= 701 && mappedData.codice_previsione <= 781) {
            ...
            <div className="gridWeekContainer m-3">
                ...
                <div key={index} className="card">   
                    <div className="card-body">
                        ...
                        <h5 className="card-text mt-2"><FontAwesomeIcon icon={faCloudRain}/> {(e.prob_pioggia).toString().slice(0,3)} Mm</h5>
                        <h5 className="card-text mt-2"><FontAwesomeIcon icon={faWind}/> {(e.velocita_vento).toString().slice(0,3)} M/s</h5>
                    </div> 
                </div>
                ))}
            </div>
